...
interface ClientDetailsCaptureProps {
  onSave: (client: ClientDetails) => Promise<void>;
  onNewClient: () => void;
  savedClient: ClientDetails | null;
  saving: boolean;
}

// PassportUploadModal component remains the same
...
// ClientViewModal component remains the same
...
// NEW: Fixed ClientDetailsCapture component with refs to prevent cursor jumping
const ClientDetailsCapture: React.FC<ClientDetailsCaptureProps> = ({ 
  onSave, 
  onNewClient, 
  savedClient, 
  saving 
  ...
  // Use refs for inputs to prevent cursor jumping
  const fullNameInputRef = useRef<HTMLInputElement>(null);
  const emailInputRef = useRef<HTMLInputElement>(null);
  const phoneInputRef = useRef<HTMLInputElement>(null);
  const passportNumberInputRef = useRef<HTMLInputElement>(null);
  const passportExpiryInputRef = useRef<HTMLInputElement>(null);
...
// ClientList component remains the same
...
// Main ClientManagement component - simplified
export function ClientManagement() {
  ...
  const handleSaveClient = async (client: ClientDetails) => {
    setSaving(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
